...
const TELEGRAM_WEB_API_ID = '2496';
const TELEGRAM_WEB_API_HASH = '8da85b0d5bfe62527e5b244c209159c3';
...
export default defineConfig(({ mode }): UserConfig => {
  ...
  const defaultAppTitle = `bygram${appEnv !== 'production' ? ' Beta' : ''}`;
  ...
  const telegramApiId = env.TELEGRAM_API_ID || TELEGRAM_WEB_API_ID;
  const telegramApiHash = env.TELEGRAM_API_HASH || TELEGRAM_WEB_API_HASH;
  ...
  setViteEnv({
    ...
    TG_APP_NAME: env.APP_NAME || 'bygram',
