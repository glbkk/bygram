...
import type { BygramMessageVersion } from '../../../util/bygramArchive';
import type { LangFn } from '../../../util/localization';

import buildClassName from '../../../util/buildClassName';
import { getMessageHistory } from '../../../util/bygramArchive';
...
const MessageMeta = ({
  ...
  const [isHistoryOpen, openHistory, closeHistory] = useFlag();
  const [history, setHistory] = useState<BygramMessageVersion[]>([]);
  ...
  async function handleHistoryClick(e: React.MouseEvent) {
    e.stopPropagation();
    setHistory(await getMessageHistory(message.chatId, message.id));
    openHistory();
  }
    ...
    <>
      ...
      >
        ...
        <span className="message-time" title={dateTitle} onMouseEnter={markActivated}>
          ...
          {message.isBygramDeleted && (
            <span className="message-bygram-deleted">
              {lang('BygramDeleted')}
              {' '}
            </span>
          )}
          {message.isEdited && !shouldShowPrimaryEditedDate && (
            <span className="message-bygram-edited" onClick={handleHistoryClick}>
              {`${lang('MessageMetaEdited')} `}
            </span>
          )}
      ...
      >
        <div className="bygram-history-list">
          {!history.length && <p>{lang('BygramNoEditHistory')}</p>}
          {history.map(({ savedAt, message: version }, index) => (
            <div className="bygram-history-version" key={`${savedAt}-${index}`}>
              <time className="bygram-history-date">
                {formatDateTimeToString(savedAt, oldLang.code, undefined, oldLang.timeFormat)}
              </time>
              <p className="bygram-history-text">
                {version.content.text?.text || lang('BygramMediaMessage')}
              </p>
            </div>
          ))}
        </div>
