...
import { shareBygramMessageShot } from '../../../util/bygramSharing';
...
const MessageContextMenu = ({
  ...
  const handleMessageShot = useLastCallback(async () => {
    const sender = message.senderId ? selectPeer(getGlobal(), message.senderId) : undefined;
    const author = (sender && getPeerTitle(lang, sender)) || userFullName || lang('BygramUnknownSender');

    try {
      await shareBygramMessageShot(message, author, {
      ...
      if (!(err instanceof DOMException) || err.name !== 'AbortError') {
        showNotification({ message: lang('BygramMessageShotError') });
      }
    }
    onClose();
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <MenuItem icon="camera" onClick={handleMessageShot}>{lang('BygramMessageShot')}</MenuItem>
