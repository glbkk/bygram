...
import { exportBygramChat } from '../../../util/bygramSharing';
...
const ChatExtra = ({
  ...
  const handleUserIdClick = useLastCallback(() => {
    copyTextToClipboard(userId!);
    showNotification({ message: lang('BygramUserIdCopied') });
  });

  const handleExportChat = useLastCallback(async () => {
    const title = getPeerTitle(lang, (user || chat)!) || chatOrUserId;
    try {
      const isExported = await exportBygramChat(chatOrUserId, title, {
        deleted: lang('BygramDeleted'),
        edited: oldLang('MessageMetaEdited'),
        media: lang('BygramMediaMessage'),
        title: lang('BygramExportChat'),
      }, (senderId) => {
        const sender = selectPeer(getGlobal(), senderId);
        return (sender && getPeerTitle(lang, sender)) || senderId;
      });
      showNotification({ message: lang(isExported ? 'BygramExportComplete' : 'BygramExportEmpty') });
    } catch {
      showNotification({ message: lang('BygramExportError') });
    }
  });
      ...
      <Island>
        {userId && (
          <ListItem
            ...
            style={createVtnStyle('userId')}
          >
            <span className="title allow-selection">{userId}</span>
            <span className="subtitle">{lang('BygramUserId')}</span>
          </ListItem>
        )}
        ...
        {!isInSettings && peerId && (
          <ListItem
            icon="download"
            className={styles.exportChat}
            multiline
            narrow
            ripple
            onClick={handleExportChat}
            style={createVtnStyle('exportChat')}
          >
            <span className="title">{lang('BygramExportChat')}</span>
            <span className="subtitle">{lang('BygramExportChatDesc')}</span>
          </ListItem>
        )}
