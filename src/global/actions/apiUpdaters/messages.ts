...
export function deleteMessages<T extends GlobalState>(
  ...
  if (chatId) {
    void markArchivedMessagesDeleted(chatId, ids);
  } else {
    ids.forEach((id) => {
      const commonBoxChatId = selectCommonBoxChatId(global, id);
      if (commonBoxChatId) {
        void markArchivedMessagesDeleted(commonBoxChatId, [id]);
      }
    });
  }

  const bygramSettings = getBygramSettings();
  if (chatId && bygramSettings.isArchiveEnabled && bygramSettings.isAntiDeleteEnabled) {
    ids.forEach((id) => {
      const message = selectChatMessage(global, chatId, id);
      if (!message) return;

      void archiveDeletedMessage(message);
      global = updateChatMessage(global, chatId, id, {
        isBygramDeleted: true,
        bygramDeletedAt: getServerTime(),
      });
    });
    setGlobal(global);

    void restoreArchivedMessages(chatId, ids);
    return;
  }
...
async function restoreArchivedMessages(chatId: string, ids: number[]) {
  const records = await getArchivedMessages(chatId, ids);
  if (!records.length) return;

  let global = getGlobal();
  records.forEach(({ message, deletedAt }) => {
    if (selectChatMessage(global, chatId, message.id)) return;

    const restoredMessage: ApiMessage = {
      ...message,
      isBygramDeleted: true,
      bygramDeletedAt: deletedAt ? Math.round(deletedAt / 1000) : getServerTime(),
    };
    global = addMessages(global, [restoredMessage]);
    global = updateListedAndViewportIds(global, restoredMessage);
  });
  setGlobal(global);
}
